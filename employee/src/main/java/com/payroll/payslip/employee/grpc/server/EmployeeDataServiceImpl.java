...
public class EmployeeDataServiceImpl extends EmployeeDataServiceGrpc.EmployeeDataServiceImplBase {
    ...
    @PostConstruct
    public void init() {
        personChannel = ManagedChannelBuilder.forAddress("localhost", 50053)
                .usePlaintext()
                .build();
    ...
    public void createEmployeeViaPerson(
        ...
        GetPersonRequest getPersonRequest =
                GetPersonRequest.newBuilder().setId(personId).build();
    ...
    public void getEmployeeById(GetEmployeeRequest request, StreamObserver<GetEmployeeResponse> responseObserver) {
        ...
        GetEmployeeResponse response =
                GetEmployeeResponse.newBuilder().setEmployee(employee).build();
        responseObserver.onNext(response);
        responseObserver.onCompleted();
    }
