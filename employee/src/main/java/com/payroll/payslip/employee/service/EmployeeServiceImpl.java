...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public CreateEmployeeResponseDto createEmployee(CreateEmployeeRequestDto request) {
        ...
        logger.info("person data: {}, {}", getPersonResponse.getFirstName(), getPersonResponse.getSureName());

        EmployeeEntity employeeEntity = new EmployeeEntity();
        employeeEntity.setPersonId(getPersonResponse.getId());
        employeeEntity.setFullName(getPersonResponse.getFirstName() + " " + getPersonResponse.getSureName());
    ...
    public UpdateEmployeeResponse updateEmployee(Long employeeId, UpdateEmployeeRequest request) {
        EmployeeEntity employeeEntity = repository
                .findById(employeeId)
                .orElseThrow(() -> new EmployeeNotFoundException("can`t find employee by id " + employeeId));
    ...
    public EmployeeResponse findEmployeeById(Long employeeId) {
        return mapper.mapToResponse(repository
                .findById(employeeId)
                .orElseThrow(() -> new EmployeeNotFoundException("can`t find employee by id " + employeeId)));
    }
