...
public class OrganizationServiceImpl implements OrganizationService {
    ...
    private final OrganizationDtoToOrganizationMapper mapper = OrganizationDtoToOrganizationMapper.INSTANCE;

    @Override
    public OrganizationResponse createOrganization(OrganizationRequest organizationRequest) {
        OrganizationEntity organization = organizationPostgresRepository.save(mapper.mapToEntity(organizationRequest));
        return mapper.mapToResponse(organization);
    }

    @Override
    public OrganizationResponse findByOrganizationId(Long organizationId) {
        OrganizationEntity organization = organizationPostgresRepository
                .findById(organizationId)
                .orElseThrow(
                        () -> new OrganizationNotFoundException("can`t find organization by id " + organizationId));
        return mapper.mapToResponse(organization);
    }

    @Override
    public OrganizationResponse findByOrganizationKod(FindOrganizationByKodRequest organizationKod) {
        return mapper.mapToResponse(organizationPostgresRepository
    ...
    public List<OrganizationResponse> findAll() {
        return organizationPostgresRepository.findAll().stream()
